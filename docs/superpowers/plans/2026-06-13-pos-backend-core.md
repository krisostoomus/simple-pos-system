...
**Carry-over notes for Plan 2 (from the final review):**
- Define whether `IProductRepository.GetByIdsAsync`/`GetAllActiveAsync` return inactive products. Today `Product.HasSufficientStock` folds in the `IsActive` check, so an inactive-but-in-stock product would surface at checkout as `InsufficientStockException`; if the repo can return inactive products, give them a distinct path/exception.
- The neutral-culture helper (`culture.Split('-')[0].ToLowerInvariant()`) is duplicated in `CatalogService` and `ReportingService`; consider extracting a shared helper and adding a region-qualified (`et-EE`) test through the service layer.
