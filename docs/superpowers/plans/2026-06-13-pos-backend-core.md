...
**Next:** Plan 2 (Backend Infrastructure + API) implements the ports (`IProductRepository`, `IOrderRepository`, `IUnitOfWork`, `IPaymentService`, `IStockNotifier`, `IReportQueries`) against EF Core + Postgres, adds the minimal API, Swagger, SignalR hub, JSON seeding, ProblemDetails error mapping (mapping each exception type to the `errorCode`s in spec §5), and integration tests including the parallel-checkout concurrency test.
